...
class SaveJson_v3FinanceTransactionsList_TransactionServices {
    static function main() {
        ...
        foreach ($array as $element) {
            ...
            foreach($element['services'] as $service) {
                ...
                $data['custom_id'] = "$operation_id.services[$j]";
                $data['operation_id'] = $operation_id;

                $str_fields = ['name', 'price'];
                ...
                foreach ($str_fields as $current_field) {
                    $data[$current_field] = "" . $service[$current_field];
                }

                foreach ($boolean_fields as $current_field) {
                    $data[$current_field] = $service[$current_field] ? '1' : '0';
                }

                foreach ($str_array_fields as $current_field) {
                    $data[$current_field] = implode(';', $service[$current_field]);
                }
        ...
        $JSON = $result_array;
        $FILE_PATH = "$HOME/database/OZN_v3FinanceTransactionsList_TransactionServices.json";
