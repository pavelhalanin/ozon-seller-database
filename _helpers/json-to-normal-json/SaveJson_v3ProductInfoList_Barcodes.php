...
class SaveJson_v3ProductInfoList_Barcodes {
    static function main() {
        ...
        for ($i = 0; $i < count($array); $i++) {
            ...
            $j = 0;
            for ($j = 0; $j < count($element['barcodes']); $j++) {
                $barcode = $element['barcodes'][$j];

                $data = [];
                $ai += 1;
                $data['custom_auto_increment'] = $ai;
                $data['custom_id'] = "[$i].barcodes[$j]";
        ...
        $JSON = $result_array;
        $FILE_PATH = "$HOME/database/OZN_v3ProductInfoList_Barcodes.json";
